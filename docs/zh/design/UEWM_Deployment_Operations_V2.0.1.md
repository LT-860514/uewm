...
**最后更新：** 2026-04-03  
**状态：** 设计完成（100% 覆盖 R05, NFR-1/2/6/9/12 + G-Space 运维 + GPU 优化部署 + 社区版部署）  
**变更历史：**
- V4.0/deliver-v1.0: SLO, 错误预算, GPU 隔离, 影子模式, HA, 混沌
- V1.0.1: GPU 优化 Helm 配置 (§15), 社区版/企业版部署 (§15.2), GPU 监控告警 (§16)
- V2.0.0: G-Space 运维 (采集器监控/存储/容量), Discovery 运维, 桥接健康监控, PoC 部署规格
- **V2.0.1: (LeWM 集成) PoC 目录含 probes/voe/sigreg 子目录; SIGReg 监控告警; 全量合并 V1.0.1 内容，消除所有引用依赖**
...
uewm-system (Brain Core + **G-Space Engine** + **Discovery Engine** + EIP Gateway), uewm-agents (12 Agent Deployments), uewm-data (PostgreSQL + Redis + Kafka + Milvus + Neo4j), uewm-monitoring (Prometheus + Grafana + OTel), uewm-vault (HashiCorp Vault), **uewm-ext-agents** (第三方 Agent 独立命名空间)。

**[deliver-v1.1]** Neo4j 新增: 用于长期记忆语义记忆知识图谱。Profile-S 单实例, Profile-M 3 节点因果一致集群, Profile-L 按分片独立部署。
...
### 11.6 G-Space 存储容量 (V2.0 新增)

| Profile | G-Space 指标/天 | 实时存储(30天) | 历史(1年) | 年增量 |
|---------|---------------|-------------|---------|--------|
| Profile-S | ~50K 数据点 | ~50 MB | ~600 MB | ~600 MB |
| Profile-M | ~500K 数据点 | ~500 MB | ~6 GB | ~6 GB |
| Profile-L | ~2M 数据点 | ~2 GB | ~24 GB | ~24 GB |

G-Space 与现有 PostgreSQL 实例共享, 独立 schema `gspace`。
...
### 12.8 G-Space + 桥接性能测试 (V2.0 新增)

| 测试项 | 负载参数 | SLO 目标 | 持续时间 |
|--------|---------|---------|---------|
| G-Space 采集 P99 | 10 项目并行 | < 500ms/采集周期 | 1h |
| φ 解码 P99 | 50 并发请求 | < 50ms | 30min |
| G-Space 查询 P99 | 100 查询/min | < 200ms (实时), < 5s (历史) | 30min |
| Discovery 事件处理 | 10 events/min | < 2s | 30min |
...
```
Phase 0A→0B 门禁 (PoC → 最小可行 Brain):
  □ ARI > 0.2 (Z-Space 有意义的聚类)
  □ φ R² > 0.2 (Z→G 解码有效)
  □ Z 超越 G 独立预测 (p < 0.05)
  □ 噪声率 < 30% (Z_NOISE 不超标)
  □ PoC 代码审查通过
...
### 15.1 混合精度与推理优化 Helm 配置

```yaml
# values.yaml GPU 优化配置 [V1.0.1]
gpu_optimization:
  mixed_precision:
    training: "bf16"           # bfloat16 训练
    inference: "fp16"          # float16 推理
    gradient_accumulation: "fp32"
  gradient_checkpointing:
    jepa_predictor: true       # 每 2 层检查点
    context_encoder: true      # 每 3 层检查点
    alignment_trainer: true    # 全量重计算
  deepspeed:
    enabled: true
    config_file: "deepspeed/zero-stage1.json"  # Phase 0
  tensorrt:
    enabled: false             # Phase 1+ 启用 (FF_TENSORRT_INFERENCE)
    precision: "fp16"
    max_batch_size: 32
  flash_attention:
  ...
  memory_budget:
    ...
    max_vram_pct: 0.85         # 留 15% headroom
```

### 15.2 社区版 vs 企业版部署 Profile

```
社区版 (Community Edition) 部署:
  Helm: values-community.yaml
  特性:
    ├── Profile-S 仅有 (单 Tenant, 1 项目)
    ├── 内环 5 Agent + Brain Core + EIP Gateway
    ├── 长期记忆: 启用 (Episode + Semantic)
    ├── 自进化: 启用 (每日 1 次)
    ├── 第三方 Agent: 启用 (free tier, ≤3 个)
    ├── Standalone API: 启用
    ├── 中环/外环 Agent: 禁用
    ├── 联邦学习: 禁用
    ├── 多租户: 禁用
  ...
  最小硬件: 
    GPU: 1× RTX 3060+ (12GB VRAM) 或 Cloud A100
    CPU: 8 核
    RAM: 32 GB
    Storage: 50 GB SSD

企业版 (Enterprise Edition) 部署:
  Helm: values-profile-m.yaml 或 values-profile-l.yaml
  特性: 全部功能启用
  硬件: 见 §3.2 资源基线
```
...
```
Phase 0A PoC 部署 (最小化, 8 weeks):

  不需要 K8s — 本地 Python 环境即可
  
  硬件: Mac M5 Max 或 RTX 3060
  存储: SQLite + 本地文件系统
  依赖: PyTorch, HuggingFace, scikit-learn, statsmodels, matplotlib
  
  目录结构:
    uewm-poc/
    ├── data/
    │   ├── raw/           # GitHub API 原始数据
    │   ├── gspace/        # G-Space Parquet 文件
    │   ├── zspace/        # Z-Space 编码向量 (256-d)  [V2.0.1]
    │   └── voe/           # VoE 测试场景 (100 scenarios)  [V2.0.1]
    ├── models/
    │   ├── encoders/      # CodeBERT checkpoints
    │   ├── projections/   # 投影头权重 (MLP+BN)  [V2.0.1]
    │   ├── phi/           # φ 解码器权重
    │   ├── probes/        # 探测头权重 (linear + MLP per metric)  [V2.0.1]
    │   └── predictor/     # JEPA predictor 权重
    ├── results/
    │   ├── gate_review/   # Gate Review 结果
    │   ├── probing/       # 逐维度探测 R² 报告  [V2.0.1]
    │   ├── voe/           # VoE ROC 曲线 + AUC  [V2.0.1]
    │   ├── sigreg/        # SIGReg 正态性检验 + 训练曲线  [V2.0.1]
    │   └── visualizations/# t-SNE, R² 图表
    └── scripts/
        ├── collect.py     # G-Space 数据采集
        ├── encode.py      # Z-Space 编码 (含投影头)  [V2.0.1]
        ├── train_phi.py   # 桥接函数训练
        ├── train_jepa.py  # JEPA predictor 训练 (含 SIGReg)  [V2.0.1]
        ├── probe.py       # 物理探测头训练+评估  [V2.0.1]
        ├── voe_test.py    # 违反预期测试  [V2.0.1]
        ├── evaluate.py    # Gate Review 自动化
        └── discover.py    # Discovery Signal 分析
```
...
| **R05 AC-10: 社区版独立部署** | **§15.2** | **Profile-S 社区版完整闭环** |
| **NFR-12: GPU VRAM 不超预算** | **§15.1+§16** | **nvidia-smi 峰值监控** |
| **R05 AC-11: G-Space 采集 P99 < 500ms** | **§12.8** | **性能测试** |
| **R05 AC-12: φ 解码 P99 < 50ms** | **§12.8** | **性能测试** |
| **R05 AC-13: PoC 可本地部署运行** | **§17** | **Mac/RTX 独立运行验证** |
| **GND-AC-1: G-Space ≥80 指标 >95% 采集率** | **§11.4** | **健康仪表盘** |
